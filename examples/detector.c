...
void dump_image_CHW(image sized, char *filename){
    ...
    for(c=0;c<sized.c;c++){
        for(y=0;y<sized.h;y++){
            for(x=0;x<sized.w;x++){
                int idx = c*sized.h*sized.w+y*sized.w+x;
                // unsigned char V=sized.data[idx]*255;
                unsigned char V=sized.data[idx]*255.;
                fprintf(fp,"%02x\n",V);
            }
        }
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        dump_image_CHW(im ,"image_DOG_BGR_3x288x352_rgbgr.bin");
        //image sized = letterbox_image(im, net->w, net->h); // For Debug,by comment it can't recognize Bicycle
        image sized = resize_image(im, net->w, net->h); // For Debug resize only without latterbox_image
