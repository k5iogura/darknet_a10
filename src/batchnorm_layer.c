...
void new_scale_bias(float * output, float *scales, float *means, float *variances, float *biases, int channels){
    static int no=0,c,j;
    char txt_file[1024], bin_file[1024];
    union {float f;unsigned char c[4];} uni1,uni2;
    sprintf(txt_file,"param_%d_new_scale_bias.txt",no);
    sprintf(bin_file,"param_%d_new_scale_bias.bin",no);
    FILE *txt=fopen(txt_file,"w");
    FILE *bin=fopen(bin_file,"w");
    for(c = 0; c<channels; c++){
        float new_scale = scales[c]/(sqrt(variances[c])+0.000001f);
        float new_bias  = -means[c]*scales[c]/(sqrt(variances[c])+0.000001f)+biases[c];
        uni1.f = new_scale;
        uni2.f = new_bias;
        fprintf(txt,"%f\n%f\n",new_scale, new_bias);
        for(j=3;j>=0;j--) fprintf(bin,"%02x",uni1.c[j]);
        fprintf(bin,",\n");
        for(j=3;j>=0;j--) fprintf(bin,"%02x",uni2.c[j]);
        fprintf(bin,",\n");
        if(no==0 && c==0){
            printf("out = new scale*bias : %f = %f * X[0] + %f\n",output[0], new_scale, new_bias);
        }
    }
    fclose(txt);
    fclose(bin);
...
void forward_batchnorm_layer(layer l, network net)
{
    ...
    if(net.train){
        ...
        if(no==0) printf("input X[0] = %f\t",l.output[0]);
        normalize_cpu(l.output, l.rolling_mean, l.rolling_variance, l.batch, l.out_c, l.out_h*l.out_w);
    }
    scale_bias(l.output, l.scales, l.batch, l.out_c, l.out_h*l.out_w);
    add_bias(l.output, l.biases, l.batch, l.out_c, l.out_h*l.out_w);
    new_scale_bias(l.output, l.scales, l.rolling_mean, l.rolling_variance, l.biases,l.out_c);
    no++;
}
