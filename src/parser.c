...
void twrite(int no, const char* pname, float* d, int num){
    int i,j;
    char filename_txt[1024], filename_bin[1024];
    union {float f;unsigned char c[4];} uni;
    sprintf(filename_txt,"param_%d_%s.txt",no,pname);
    sprintf(filename_bin,"param_%d_%s.bin",no,pname);
    FILE *fp_txt=fopen(filename_txt,"w");
    FILE *fp_bin=fopen(filename_bin,"w");
    for(i=0;i<num;i++){
        uni.f = d[i];
        fprintf(fp_txt,"%15.10f,\n",uni.f);
        for(j=3;j>=0;j--) fprintf(fp_bin,"%02x",uni.c[j]);
        fprintf(fp_bin,",\n");
    }
    fclose(fp_txt);
    fclose(fp_bin);
...
void load_convolutional_weights(layer l, FILE *fp)
{
    ...
    fread(l.biases, sizeof(float), l.n, fp);
    twrite(no, "biases",  l.biases, l.n);
    if (l.batch_normalize && (!l.dontloadscales)){
        fread(l.scales, sizeof(float), l.n, fp);
        fread(l.rolling_mean, sizeof(float), l.n, fp);
        fread(l.rolling_variance, sizeof(float), l.n, fp);
        twrite(no, "scales",  l.scales,           l.n);
        twrite(no, "mean",    l.rolling_mean,     l.n);
        twrite(no, "variance",l.rolling_variance, l.n);
    }
    fread(l.weights, sizeof(float), num, fp);
    twrite(no, "weight",l.weights, num);
    no++;
